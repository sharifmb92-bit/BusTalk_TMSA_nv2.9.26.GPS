...
function busTalkApp() {
    return {
        ...
        // AUTO "EN PARADA" INSTANTÁNEO POR VELOCIDAD < 5 KM/H EN LA PRÓXIMA MARQUESINA
        comprobarLlegadaMarquesina(lat, lng, velocidadKmh) {
            const siguienteDestino = this.obtenerDestinoSiguiente();

            if (!siguienteDestino || this.enParada) {
                return;
            }
            ...
            // Si pasa de largo a más de 15 km/h actualizamos ubicación y salimos a ruta inmediatamente
            if (velocidadKmh > 15) {
            ...
            // DISPARO INSTANTÁNEO AL CAER POR DEBAJO DE 5 KM/H EN LA GEOCERCA
            if (velocidadKmh < 5 && this.miUbicacion !== siguienteDestino) {
                this.actualizarUbicacion(siguienteDestino);
            }
        },
